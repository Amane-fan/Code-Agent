...
Code-Agent 支持内置 skills。每个 skill 位于 Code-Agent 自身受控目录下的独立子目录中，并使用
`SKILL.md`：
...
启动时只把 `name` 和 `description` 注入系统上下文。模型需要完整内容时，会调用：

```text
<action>{"tool":"load_skill","args":{"name":"python"}}</action>
```

`load_skill` 的 observation 会把完整 `SKILL.md` 放入后续模型上下文。目标 workspace 中的文件不会被当作
Code-Agent skill 或系统提示词加载。
